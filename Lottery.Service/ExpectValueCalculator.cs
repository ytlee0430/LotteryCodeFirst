...
namespace Lottery.Service
{
    ...
    {
        public Tuple<double, double> CalculateExpectValue(List<LotteryRecord> data, IAnalyzer analyzer, int expectValueCount, int variableOne)
        {
            ...
            for (int i = 1; i <= expectValueCount; i++)
            {
                ...
                var currentResult = analyzer.Analyze(data, variableOne)
                    .Where(a => !a.IsSpecial).Take(6);

                var currentSpecialResult = analyzer.Analyze(data, variableOne)
                    .Where(a => a.IsSpecial).Take(1);
